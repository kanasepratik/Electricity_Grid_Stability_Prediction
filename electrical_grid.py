...
def electrical_grid():
    if request.method == 'POST':
        ...
        if prediction[0][0] >= 0:
            return render_template('electrical_grid.html', 
            	prediction_text="Oops! the system is linearly unstable with a stability value of {:.5f}.".format(prediction[0][0]), 
            	title='Electrical Grid Stability')
        else:
            return render_template('electrical_grid.html', 
            	prediction_text="Great! the system is stable with a stability value of {:.5f}.".format(prediction[0][0]), 
            	title='Electrical Grid Stability')
    else:
